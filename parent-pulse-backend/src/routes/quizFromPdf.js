...
router.post('/quiz-from-pdf', upload.single('file'), async (req, res) => {
    try {
        const prompt = req.body.prompt || 'Using this PDF, quiz me on the material. Ask me questions one by one and assess my answer.';
        ...
        const uploaded = await openai.files.create({
            file: new File([pdfBuffer], fileName, { type: req.file?.mimetype || 'application/pdf' }),
            purpose: 'assistants'
        });
